...
```{r}
reprex_1_df <- dplyr::tribble(~common_name, ~scientific_name, ~size,
                              "Cork Oak", "Quercus suber", "MEDIUM",
                              "Coast Live Oak", "Quercus agrifolia", "LARGE")

# Below the code within `mutate()` is not running properly because the function `tolower` being encased within the parenthesis of the function `is.character()`
reprex_1_fix <- reprex_1_df %>% 
  mutate(across(where(is.character, tolower))) 
```
...
reprex_2_df <- dplyr::tribble(~tree_width, ~tree_height,
                       100, 80,
                       150, 110,
                       120, 90
                       )

# This graph is not currently working because the "shape" is not a variable from the data set and therefore should not be within `aes()`
ggplot(reprex_2_df, aes(x = tree_width, y = tree_height)) +
         geom_point(aes(shape = 12), color = "purple")
```
